-- ============================================================
-- MIGRATION — PHASE 6: Analytics functions
-- Run in Supabase Dashboard > SQL Editor > New query > Run
-- These aggregate research data fast, inside the database.
-- Access control: only the class's teacher or an admin gets rows.
-- ============================================================

-- Per-student statistics for one class
create or replace function public.class_student_stats(p_class_id uuid)
returns table (
  student_id uuid,
  total_questions bigint,
  correct_answers bigint,
  first_try_correct bigint,
  total_hints bigint,
...
as $$
  ...
  left join public.questions q
   ...
   and q.question_type <> 'lesson_content'
...
-- Per-word statistics for one class (which words are hardest?)
...
as $$
  ...
  left join public.questions q
   ...
   and q.question_type <> 'lesson_content'
   and q.student_id in
       (select student_id from public.enrollments where class_id = p_class_id)
  where (
      exists (select 1 from public.classes cl
